...
bool TGALoader::Load(const char* path)
{
    ...
    fileSize = static_cast<int>(inFile.tellg()) - headerSize;
    //Get currents position in file which gives us total file size, so we take the header size off
    ...
    width = (ucharCast(tempHeaderData[13]) << 8u) + ucharCast(tempHeaderData[12]);
    //Find the width (Combines two bytes into a short)
    ...
    bool flipped = false;
    if ((tempHeaderData[11] << 8) + tempHeaderData[10] == 0)
        flipped = true;
    ...
    if (type == 2) //Type 2 is RGB
    {
        ...
        else
        {
            ...
            gluBuild2DMipmaps(GL_TEXTURE_2D, 4, width, height, GL_BGR_EXT, GL_UNSIGNED_BYTE, tempTextureData);
