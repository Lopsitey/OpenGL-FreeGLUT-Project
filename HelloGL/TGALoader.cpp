...
bool TGALoader::Load(const char* path)
{
    ...
    inFile.read(tempHeaderData, length(tempHeaderData));//Read in all of the header data (the first 18 bytes)

    inFile.seekg(0, ios::end);//Seek to end of file
    fileSize = (int)inFile.tellg() - length(tempHeaderData);//Get currents position in file which gives us total file size so we take the header size off

    tempTextureData = new char[fileSize]; //Create an new aray to store data
    inFile.seekg(length(tempHeaderData), ios::beg); //Seek back to beginning of file + 18
    ...
    _height = ((unsigned char)tempHeaderData[15] << 8u) + (unsigned char)tempHeaderData[14];//Find the height the u is used to explicitly state that the shift taking place is unsigned
